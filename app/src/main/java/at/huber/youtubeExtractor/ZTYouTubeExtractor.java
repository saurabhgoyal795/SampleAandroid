...
public abstract class ZTYouTubeExtractor extends AsyncTask<String, Void, SparseArray<YtFile>> {
    ...
    public static String BASE_URL = "www.youtube.com/get_video_info?html5=1&c=TVHTML5&cver=6.20180913&video_id=";
    ...
    private static final Pattern patFragmentUrl = Pattern.compile("\"fragment_base_url\"\\s*:\\s*\"(.+?)\"");
    ...
    private static final Pattern patCipherFragmentUrl = Pattern.compile("fragment_base_url=(.+?)(\\\\\\\\u0026|\\z)");
    ...
    public ZTYouTubeExtractor(@NonNull Context con) {
    ...
    private SparseArray<YtFile> getStreamUrlsNew(){
        try{
            String ytInfoUrl = "https://asia-south1-online-zonetech-app.cloudfunctions.net/getVideoData?videoId="+videoID;
            String streamMap;
            ...
            while (mat.find()) {
                try{
                    ...
                    if(url.startsWith("https://manifest")){
                        continue;
                    }
                    ...
                    if (FORMAT_MAP.get(itag) == null) {
                        continue;
                    } else if (FORMAT_MAP.get(itag).getExt().equals("webm")) {
                        continue;
                    }
    ...
    private SparseArray<YtFile> getStreamUrls() throws IOException, InterruptedException {
        ...
        String baseUrl = Preferences.get(refContext.get(), Preferences.KEY_CUSTOM_EXTRACTOR_URL, BASE_URL);
        String ytInfoUrl = (useHttp) ? "http://" : "https://";
        ytInfoUrl += baseUrl + videoID + "&eurl="
